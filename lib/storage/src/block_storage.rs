...
impl BlockStorage for LocalBlockStorage {
    ...
    fn write_block(&self, id: &BlockId, data: &[u8]) -> Result<(), StorageError> {
        ...
        let mut data_file = &self.current_block.as_ref()
            .unwrap()
            .lock()
            .unwrap()
            .data_file;
    ...
    fn write_block_meta(&self, block_id: &BlockId, block_meta: &BlockMeta) -> Result<(), StorageError> {
        ...
        let json_string = serde_json::to_string(block_meta)
            .unwrap();
    ...
    fn read_block(&self, block_id: &BlockId, offset: u64, size: u32) -> Result<Vec<u8>, StorageError> {
        ...
        if offset + size as u64 > block_file_size {
            return Err(StorageError::CorruptedEntry(format!("Reading more data than block has. Block size: {}", block_file_size).into()));
        }
    ...
    fn read_block_index(&self, block_id: &BlockId) -> Result<BlockIndex, StorageError> {
        ...
        while offset + block_size <= file_size {
            ...
            let index_entry = BlockIndexEntry::deserialize(&buffer)
                .expect("Can not deserialize index entry");
